...
function Sidebar({ forge, account }) {
    ...
    const mint = async () => {
        try {
            ...
            alert(
                "1 minute cooldown period. Try minting again after 1 minute",
                e
            );
        }
    };
    ...
    const trade = async () => {
        if (selectedItem === selectedToken) {
            alert(
                "Same token selected, select different token! Ignore if selected different."
            );
                ...
                <div className="h-screen w-full flex justify-around items-center">
                    <div className="backdrop-blur bg-white bg-opacity-20 w-2/3 h-3/4 flex fixed">
                        <div className="w-2/3 flex flex-col items-center justify-around">
                            <img
                                src={`https://bafybeifyyyoopunxk472r55puztc45m4dlurdq2lzl6vmgxfzxjzsuixly.ipfs.dweb.link/${selectedItem}.jpg`}
                                alt="Image"
                                className="h-3/4 w-3/4 rounded-md"
                            />
                            <h1 className="text-2xl font-bold text-white">
                                {idItems[selectedItem]}
                            </h1>
                        </div>
                        <div className="bg-opacity-50 w-1/3 font-extrabold text-xl overflow-hidden">
                            {selectedItem < 3 ? (
                                <>
                                    <div
                                        className="flex items-center justify-center bg-opacity-60 h-1/4 content-center bg-green-300 cursor-pointer"
                                        onClick={() => mint()}
                                    >
                                        Mint
                                    </div>
                            ...
                            <div className="flex items-center justify-center bg-opacity-60 h-1/4 content-center bg-yellow-300">
                                Balance: {balances[selectedItem]}
                            </div>
                            <hr />
                            ...
                            <div
                                className="flex items-center justify-center bg-opacity-60 h-1/4 content-center bg-red-400 cursor-pointer"
                                onClick={() => burn()}
                            >
                                Burn
                            </div>
            ...
            <div className="w-32 backdrop-blur bg-white bg-opacity-15">
                <ul className="h-full flex flex-col justify-around items-center">
                    {[...Array(7).keys()].map((itemId) => (
                        <li
                            className="pt-1 w-5/6 cursor-pointer text-center"
                            key={itemId}
                            onClick={() => setSelectedItem(itemId)}
                        >
                            <img
                                src={`https://bafybeifyyyoopunxk472r55puztc45m4dlurdq2lzl6vmgxfzxjzsuixly.ipfs.dweb.link/${itemId}.jpg`}
                                alt={`ItemId ${itemId}`}
                            ...
                            <label
                                for="item"
                                className="text-white w-28 font-bold "
                            >
                                {idItems[itemId].split(" ")[0]}{" "}
